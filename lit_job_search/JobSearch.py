...
def find_jobs_from(website, job_title, location, desired_characs='titles links companies date_listed'):
    """
    ...
    print(load_monster_jobs(job_title, location))
    ...
    result = []
    for i in range(len(jobs_list['titles'])):
        result.append({
            'titles': jobs_list['titles'][i],
            'companies': jobs_list['companies'][i],
            'links': jobs_list['links'][i],
            'date_listed': jobs_list['date_listed'][i],
        })
...
def load_monster_jobs(job_title, location):
    getVars = {'query': job_title, 'locations': location}
    agent = {"User-Agent": 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                           'Chrome/59.0.3071.115 Safari/537.36'}
    url = ('https://www.monsterindia.com/srp/results?' + urllib.parse.urlencode(getVars))
    print(url)
    page = requests.get(url, headers=agent)
    soup = BeautifulSoup(page.content, "html.parser")
    print(soup)
    cards = soup.findAll('div', class_='card-panel apply-panel job-apply-card')
    print(cards)
